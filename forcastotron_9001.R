...
weekend_dates <- seq.Date(as.Date("2018-07-27"),as.Date("2018-07-29"),by="days")
...
#start_date <- as.Date("2018-06-08")
#end_date <- as.Date("2018-06-13")
if(as.Date(last_date) < (Sys.Date()-1)) {
  date_list <- seq.Date(start_date,end_date,by="days")
  box_office <- boxoffice(date_list,site="numbers")
...
#fml_table$is_split <- grepl("(^FRI: )|(^SAT: )|(^SUN: )",fml_table$movie)
#fml_table$movie <- gsub("(^FRI: )|(^SAT: )|(^SUN: )","",fml_table$movie)

#updated_boxoffice$movie <- replace_curly_quote(updated_boxoffice$movie)
